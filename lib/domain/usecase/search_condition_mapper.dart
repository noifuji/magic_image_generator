...
class SearchConditionMapper {
  ...
  static SearchCondition map(List<String> raw) {
    ...
    if (keyword == SearchKeywordType.color) {
      ...
      if (colorMap[raw.last.toLowerCase()] != null) {
      ...
      } else if (isColorLetterOnly(raw.last.toLowerCase()) &&
          raw.last.length <= 5) {
        var colors = colorMap.keys.toList();

        for (var i = 0; i < raw.last.length; i++) {
          colors =
              colors.where((element) => element.contains(raw.last[i])).toList();
        }
        ...
        value = colorMap[
            colors.firstWhere((element) => element.length == raw.last.length)];
      } else {
        throw Exception();
      }
    ...
    relationalOperatorType =
        relationalOperatorMap[raw.sublist(1, raw.length - 1).join()]!;
  ...
  static bool isMappable(List<String> raw) {
    ...
    if (!relationalOperatorMap.keys
        .toList()
        .contains(raw.sublist(1, raw.length - 1).join())) {
      return false;
    }
    return true;
  }
  ...
  static bool isColorLetterOnly(String str) {
    ...
    for (var i = 0; i < str.length; i++) {
      isColorLetter =
          isColorLetter && ["w", "u", "b", "r", "g"].contains(str[i]);
    }
