...
class CanvasViewScreen extends StatefulWidget {
  ...
  const CanvasViewScreen({Key? key, required this.responsiveColumns, required this.responsiveColumnWidth, required this.responsiveGutterWidth})
      : super(key: key);
...
class CanvasViewScreenState extends State<CanvasViewScreen> {
  ...
  Widget build(BuildContext context) {
    List<List<CardInfoHeader>> selectedCardMatrix = Provider.of<CanvasViewModel>(context).selectedCards;
    ...
    return LayoutBuilder(builder: (BuildContext context, BoxConstraints constraints) {
      _canvasViewZoomRatio = min(constraints.maxWidth / (matrixWidth + 5), (constraints.maxHeight) / (matrixHeight + constants.rawCardImageHeight));
      ...
      draggableImageMatrix = selectedCardMatrix.map((row) {
        ...
        List<Widget> cardWidgets = row.map((card) {
          ...
          return DragTarget<Map<String, int>>(
              ...
              onAccept: (data) => Provider.of<CanvasViewModel>(context, listen: false).moveCard(data, {"row": rowIndex, "col": colIndex}),
              onWillAccept: (data) => data is Map<String, int> && data.keys.contains("row") && data.keys.contains("col"));
        }).toList();
        ...
        cardWidgets.add(DragTarget<Map<String, int>>(
          ...
          onAccept: (from) => Provider.of<CanvasViewModel>(context, listen: false).moveCard(from, {"row": rowIndex, "col": row.length}),
        ...
        SizedBox(
            ...
            child: SingleChildScrollView(
              ...
              SizedBox(
                  ...
                  child:
                      RepaintBoundary(key: _globalKey, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: draggableImageMatrix))),
              DragTarget<Map<String, int>>(
                ...
                onAccept: (from) => Provider.of<CanvasViewModel>(context, listen: false).moveCard(from, {"row": selectedCardMatrix.length, "col": 0}),
                onWillAccept: (data) => data != null && data["row"] != null && data["col"] != null,
              )
            ]))),
        SizedBox(
